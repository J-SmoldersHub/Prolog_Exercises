...
Eventually, yes it would.
The Escape clause is never called.
It will find every descendant though.
...
hasSmallerDoll(katarina, olga).
hasSmallerDoll(lga, natascha).
hasSmallerDoll(natascha, irida).
...
travelFroTo(X, Y) :- directTrain(X,Y).
travelFromTo(X, Y) :-
        directTrain(X,Z),
        travelFromTo(Z, Y).
